...
invoices, and bank lines into a state your Tax Advisor (Steuerberater) can actually use.

## The problem

Every quarter, the same ritual: dig through email for invoices, match them against
bank lines, guess at categories, and send your Tax Advisor a folder they'll have
...
- has a Tax Advisor who does the actual books
...
## The second audience: their Tax Advisor

The Tax Advisor doesn't pay for FiBuKI. But they can veto it, so they matter.

A client can invite their Tax Advisor into FiBuKI — free, read-only, no setup.
...
3. **Your Tax Advisor gets clean data** — a correct BMD export, or an invite to
   look directly.
...
If you're a Tax Advisor with a duty of professional secrecy
...
- **Not a bookkeeping system.** Your Tax Advisor does the books. We feed them.
...
Tax Advisor as a *buyer* instead of as a *gatekeeper* — that's how you end up
