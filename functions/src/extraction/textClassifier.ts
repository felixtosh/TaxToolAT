 ...
 */
export async function classifyDocumentByText(
  fileBuffer: Buffer,
  fileType: string
  ...
  if (hasCurrency) signals.push(`Currency: ${currencyCount}`);
  if (hasVat) signals.push(`VAT: ${vatCount}`);
  if (hasAmounts) signals.push(`Amounts: ${amountCount}`);
  if (hasInvoiceKeywords) signals.push(`Keywords: ${keywordCount}`);
  if (hasIban) signals.push("Has IBAN");

  // Scoring logic
  let invoiceScore = 0;

  // Strong positive signals
  if (hasInvoiceKeywords) invoiceScore += 3;
  if (hasVat) invoiceScore += 2;
  if (hasCurrency && hasAmounts) invoiceScore += 2;
  if (hasIban) invoiceScore += 1;

  // Strong negative signals
  if (nonInvoiceCount >= 2) invoiceScore -= 4;
  else if (nonInvoiceCount >= 1) invoiceScore -= 2;

  // Determine classification
  let isLikelyInvoice: boolean;
 ...
 */
export function shouldUseTextClassification(
