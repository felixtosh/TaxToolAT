...
exports.TRANSACTION_MATCH_CONFIG = void 0;
exports.scoreTransactionMatch = scoreTransactionMatch;
exports.shouldAutoMatchTransaction = shouldAutoMatchTransaction;
exports.logMatchScore = logMatchScore;
exports.TRANSACTION_MATCH_CONFIG = {
 ...
 */
function scoreTransactionMatch(file, transaction) {
    const breakdown = {
        amountScore: 0,
        dateScore: 0,
        partnerScore: 0,
    ...
    if (file.extractedDate) {
        const result = calculateDateScore(file.extractedDate.toDate(), transaction.date.toDate());
    ...
    if (file.extractedText && transaction.reference) {
        const result = calculateReferenceScore(file.extractedText, transaction.reference, breakdown.dateScore);
    ...
    const confidence = breakdown.amountScore +
        breakdown.dateScore +
 ...
 */
function extractInvoicePatterns(text) {
    if (!text)
        return [];
    const patterns = [];
 ...
 */
function calculateFilenameScore(fileName, transactionName) {
    ...
    for (const filePattern of filePatterns) {
        ...
        for (const txPattern of txPatterns) {
            ...
            if (filePattern.includes(txPattern) ||
 ...
 */
function shouldAutoMatchTransaction(confidence) {
    return confidence >= exports.TRANSACTION_MATCH_CONFIG.AUTO_MATCH_THRESHOLD;
}
/**
 * Log match score breakdown for debugging
 */
function logMatchScore(fileId, fileName, score) {
    const { breakdown, confidence, matchSources } = score;
    console.log(`[PrecisionSearch] Match score for file ${fileName} (${fileId}): ` +
...
//# sourceMappingURL=transactionMatcher.js.map