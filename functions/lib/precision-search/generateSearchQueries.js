...
exports.generateTypedSearchQueries = generateTypedSearchQueries;
 ...
 */
function generateTypedSearchQueries(transaction, partner, maxQueries = 8) {
    const suggestions = new Map();
    let order = 0;
    const normalizeQuery = (s) => s.trim().replace(/\s+/g, " ").toLowerCase();
    const addSuggestion = (s, type, score) => {
    ...
    if (partner?.fileSourcePatterns?.length) {
        ...
        for (const pattern of sortedPatterns.slice(0, 3)) {
            const normalized = pattern.sourceType === "local"
 ...
 */
function generateSearchQueries(transaction, partner, maxQueries = 8) {
    return generateTypedSearchQueries(transaction, partner, maxQueries)
        .map((s) => s.query);
}
//# sourceMappingURL=generateSearchQueries.js.map