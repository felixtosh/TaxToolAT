...
exports.GREAT_MATCH_COUNT = exports.GREAT_MATCH_THRESHOLD = exports.ATTACHMENT_AUTO_CONNECT_THRESHOLD = exports.ATTACHMENT_MATCH_THRESHOLD = exports.RECEIPT_KEYWORDS = void 0;
exports.scoreAttachmentMatch = scoreAttachmentMatch;
// Receipt/invoice keywords (multilingual)
exports.RECEIPT_KEYWORDS = [
    "invoice",
    "rechnung",
    "receipt",
    "beleg",
...
// Helper functions
function buildAmountVariants(amountCents) {
    if (amountCents == null)
        return [];
 ...
 */
function scoreAttachmentMatch(input) {
    const { filename, mimeType, emailSubject, emailFrom, emailSnippet, emailBodyText, emailDate, integrationId, fileExtractedAmount, fileExtractedDate, fileExtractedPartner, transactionAmount, transactionDate, transactionName, transactionReference, transactionPartner, partnerName, partnerEmailDomains, partnerFileSourcePatterns, } = input;
    ...
    const gmailPatterns = (partnerFileSourcePatterns || []).filter((pattern) => pattern.sourceType === "gmail" && pattern.integrationId);
    ...
    if (fileExtractedPartner && (partnerName || transactionPartner)) {
        ...
        const targetPartners = [partnerName, transactionPartner]
            .filter(Boolean)
            .map((p) => p.toLowerCase());
        const hasMatch = targetPartners.some((p) => filePartnerLower.includes(p) || p.includes(filePartnerLower));
        if (hasMatch) {
    ...
    if (transactionDate && emailDate) {
        const dayDiff = Math.abs(emailDate.getTime() - transactionDate.getTime()) / (1000 * 60 * 60 * 24);
 ...
 */
exports.ATTACHMENT_MATCH_THRESHOLD = 60; // Download/suggest at 60%+
/**
 * Minimum score threshold for auto-connecting
 */
exports.ATTACHMENT_AUTO_CONNECT_THRESHOLD = 75; // Auto-connect at 75%+
/**
 ...
 */
exports.GREAT_MATCH_THRESHOLD = 75; // Stop searching at 75%+
/**
 * Number of great matches needed to stop searching
 * Previously would stop with just 1 match at 80%, now requires 2 at 75%
 */
exports.GREAT_MATCH_COUNT = 2;
//# sourceMappingURL=scoreAttachmentMatch.js.map