...
exports.geminiValidateDomainOwnership = geminiValidateDomainOwnership;
const lookupCompany_1 = require("./lookupCompany");
 ...
 */
async function geminiValidateDomainOwnership(domain, companyName, userId) {
...
- "receipts.uber.com" for "Uber B.V." -> {"isOwner": true, "confidence": 90, "reason": "Subdomain of Uber"}`;
    try {
        ...
        if (parsed.confidence < MIN_CONFIDENCE_THRESHOLD) {
            console.log(`[DomainValidation] Low confidence (${parsed.confidence}%) for "${domain}" / "${companyName}": ${parsed.reason}`);
            return {
                isOwner: false,
        ...
        console.log(`[DomainValidation] "${domain}" for "${companyName}": ` +
...
//# sourceMappingURL=validateDomainOwnership.js.map