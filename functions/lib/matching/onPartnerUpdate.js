...
function hasMatchingFieldsChanged(before, after) {
    ...
    if (before.website !== after.website)
        return true;
    // VAT ID changed
    if (before.vatId !== after.vatId)
        return true;
    // IBANs changed
    if (JSON.stringify(before.ibans || []) !== JSON.stringify(after.ibans || []))
        return true;
    // Email domains changed
    if (JSON.stringify(before.emailDomains || []) !== JSON.stringify(after.emailDomains || []))
        return true;
    return false;
}
/**
 ...
 */
async function reMatchFilePartner(fileDoc, userPartners, globalPartners) {
    ...
    let newPartnerId = previousPartnerId;
    if (topMatch && (0, filePartnerMatcher_1.shouldAutoApply)(topMatch.confidence)) {
        ...
        if (topMatch.partnerId !== previousPartnerId) {
            ...
            console.log(`[PartnerUpdate] Re-matched file ${fileDoc.id} to partner ${topMatch.partnerId} ` +
 ...
 */
async function reMatchOrphanedFilesAfterDeletion(userId, deletedPartnerId, deletedPartnerName) {
    try {
        ...
        if (unmatchedFilesSnapshot.empty) {
            console.log(`[PartnerUpdate] No orphaned files to re-match after deleting "${deletedPartnerName}"`);
            return;
        }
        console.log(`[PartnerUpdate] Found ${unmatchedFilesSnapshot.size} orphaned files to re-match ` +
        ...
        console.log(`[PartnerUpdate] Re-matching after "${deletedPartnerName}" deletion complete: ` +
            `${reMatched} re-matched to new partners, ${stillUnmatched} still unmatched`);
    }
    catch (error) {
        console.error(`[PartnerUpdate] Error re-matching files after partner ${deletedPartnerId} deletion:`, error);
    }
}
// === Main Trigger ===
exports.onPartnerUpdate = (0, firestore_1.onDocumentUpdated)({
    ...
    if (wasJustDeleted) {
        ...
        console.log(`[PartnerUpdate] Partner "${before.name}" (${partnerId}) was deleted, re-matching orphaned files`);
    ...
    console.log(`[PartnerUpdate] Partner "${after.name}" (${partnerId}) updated, re-evaluating files`);
    try {
        ...
        console.log(`[PartnerUpdate] Found ${autoMatchedFilesSnapshot.size} auto-matched + ` +
        ...
        console.log(`[PartnerUpdate] Partner "${after.name}" update complete: ` +
...
//# sourceMappingURL=onPartnerUpdate.js.map