...
function checkPatternSafety(pattern, matchCount, totalTransactions, sourceTransactionCount) {
    ...
    if (patternParts.length > 0) {
        const allPartsGeneric = patternParts.every((part) => GENERIC_BANKING_TERMS.some((term) => 
 ...
 */
function buildVerificationPrompt(partnerName, proposedPatterns, dryRunResults, totalTransactions) {
 ...
 */
async function dryRunPatternMatch(userId, partnerId, proposedPatterns, partnerNameMap) {
    ...
    try {
        ...
        if (candidatePatterns.length > 0) {
            ...
            for (const cp of candidatePatterns) {
                ...
                const safety = checkPatternSafety(cp.pattern, matches.length, totalTransactions, assignedTransactions.length);
                if (safety.rejected) {
                ...
                if (verifyText) {
                    try {
                        ...
                        verifiedPatterns = safePatterns.filter((cp) => {
                            ...
                            if (!verification)
                                return true; // Keep if not mentioned (default approve)
        ...
        const learnedPatterns = verifiedPatterns.map((p) => ({
            pattern: p.pattern,
