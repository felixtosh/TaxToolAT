...
exports.onTransactionsImported = void 0;
 ...
 */
async function getImportTransactionDateRange(importId, userId) {
 ...
 */
async function getActiveGmailIntegrations(userId) {
 ...
 */
async function hasPendingSync(integrationId) {
 ...
 */
exports.onTransactionsImported = (0, firestore_1.onDocumentCreated)({
    ...
    console.log(`[GmailSyncAfterImport] Import ${importId} created for user ${userId} ` +
    ...
    console.log(`[GmailSyncAfterImport] Import date range: ${transactionRange.minDate.toISOString()} - ` +
    ...
    for (const integration of integrations) {
        ...
        if (gaps.length === 0) {
            console.log(`[GmailSyncAfterImport] Integration ${integration.id} already covers import date range`);
            continue;
        }
        ...
        for (const gap of gaps) {
            await db.collection("gmailSyncQueue").add({
                ...
                type: "auto",
                status: "pending",
                dateFrom: firestore_2.Timestamp.fromDate(gap.from),
                dateTo: firestore_2.Timestamp.fromDate(gap.to),
                emailsProcessed: 0,
                filesCreated: 0,
            ...
            console.log(`[GmailSyncAfterImport] Queued auto sync for ${integration.email}: ` +
                `${gap.from.toISOString()} - ${gap.to.toISOString()}`);
            totalQueued++;
        }
    }
    console.log(`[GmailSyncAfterImport] Completed for import ${importId}: ${totalQueued} syncs queued`);
});
//# sourceMappingURL=onTransactionsImported.js.map