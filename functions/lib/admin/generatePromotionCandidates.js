...
exports.generatePromotionCandidates = void 0;
    ...
    try {
        ...
        const partners = partnersSnapshot.docs
            ...
            return {
                ...
                userId: data.userId,
                name: data.name,
                normalizedName: (0, partner_matcher_1.normalizeCompanyName)(data.name),
                aliases: data.aliases || [],
                ibans: data.ibans || [],
                normalizedIbans: (data.ibans || []).map((iban) => (0, partner_matcher_1.normalizeIban)(iban)),
                vatId: data.vatId,
        ...
        for (const [vatId, partnerList] of vatGroups) {
            ...
            const alreadyGrouped = groups.some((g) => g.partners.some((p) => partnerList.some((pl) => pl.id === p.id)));
        ...
        for (let i = 0; i < partners.length; i++) {
            for (let j = i + 1; j < partners.length; j++) {
                ...
                const alreadyGrouped = groups.some((g) => g.partners.some((p) => p.id === p1.id) &&
                ...
                const similarity = (0, partner_matcher_1.calculateCompanyNameSimilarity)(p1.name, p2.name);
                if (similarity >= 80) {
                    // Find or create a name-based group
                    const existingGroup = groups.find((g) => g.matchType === "name" &&
        ...
        const addedPartnerIds = new Set();
        for (const group of groups) {
            if (group.userIds.size < 2)
                continue;
            ...
            const representative = group.partners.reduce((best, current) => {
                const bestScore = (best.ibans.length > 0 ? 2 : 0) +
                    (best.vatId ? 2 : 0) +
                    (best.website ? 1 : 0) +
                    best.aliases.length;
                const currentScore = (current.ibans.length > 0 ? 2 : 0) +
                    (current.vatId ? 2 : 0) +
                    (current.website ? 1 : 0) +
        ...
        for (const partner of partners) {
            if (addedPartnerIds.has(partner.id))
                continue;
        ...
        throw new https_1.HttpsError("internal", `Failed to generate promotion candidates: ${errorMessage}`);
    }
});
//# sourceMappingURL=generatePromotionCandidates.js.map